...
public class MsIoContainer {
    ...
    private final static String ClassLabel = "className";
    /**
     * 映射缓存池
     */
    ...
    public boolean addMapping(Class clazz) throws NoSuchMethodException,InstantiationException,IllegalAccessException{
        ...
        for (Field field : fields) {
            ...
            if(!StringUtils.isEmpty(annotation.methodName())){
                try {
                    Method method = transFunctionContainer.getClass().getDeclaredMethod(annotation.methodName(), Object.class);
     ...
     * @return 是否成功
     */
    public boolean addMapping(Map<String,LinkedHashMap<String,String>> jsonData){
        if(jsonData.isEmpty()){
            return false;
        }
        for (Map.Entry<String, LinkedHashMap<String, String>> item : jsonData.entrySet()) {
            ...
            //若有该字段，则标识这个映射对象为一个类（配置文件配置的类）
            information.containsKey(ClassLabel){

            }
            information.forEach((egName,cnName) ->{
                Information infor = new Information();
                if()
                mappingItem.put(egName,infor);
            });
        }
        return true;
    }

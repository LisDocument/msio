...
### 代码配置
引入组件需要实现AbstractMsConfigure进行配置，全文理应仅此一个配置入口类。

```java
@Configuration
//此处是直接拉包，省略多个同包下的class多次引入
@MsPackageScan("com.github.lisdocument.msio")
public class MsConfigure extends AbstractMsConfigure {
    ...
    public MsIoContainer configContainer(MsIoContainer container) throws Exception {
        container.addMappings(ForwardingRecode.class,User.class,ComplexUser.class);
...
通过对1.0.0版本的版本更新，修复1.0.0的若干bug，添加下载日志发送功能

### 添加的功能项

  1.配置总入口添加方法configDataSource()：配置数据源或者文件源等返回，这边留下较大的自主性，可通过实现AbstactStoreRecordConfigure类对转储方式进行自定义，configDataSource返回的参数视AbstactStoreRecordConfigure的泛型类而定，如果返回空的情况下，记录下载功能会失效。
...
#version 1.0.1 修改部分
#是否开启上传下载记录储存(暂时未实现)
#spring.msIo.dbLog=true-----此处被删除，使用入口类的方法进行判断
#默认实现的日志存库操作的表名，当前仅仅可定义表名，字段名不可被定义
spring.msIo.tableName=tableName
#version 1.0.1 结束修改
...
6. @MsReturnTranslator:在需要导出的controller的方法上进行注解，非必要注解（不注解也可以导出），当然，这并不是无效的，其中的value属性时一个简单实现的EL表达式，用于提取返回的如果不是List的情况下进行格式调整，如被封装在一个Page对象中的List，就可以通过这个value进行提取，如下例：**@MsReturnTranslator("getPage()#getList($int$5,$int$150,wowowo,$double$1.41)")**
   ...
   id是指定的导出id集，复杂导出时必须指定，设计为String[]类型为导出多种类型的组合单次导出（多页非相同的映射）
   isComplex是否为复杂项导出，在需要复杂导出的时候必须要指定该属性为true。
   <font color=#FF0000>logSign为是否发送日志的标志，</font>
7. <font color=#FF0000>@MsTranslateOperator:功能上实现了MsReturnTranslator中EL表达式，为应对多个接口返回相同类型的数据，需要写多次相同的表达式，为解决此类问题，该注解可注解在返回的vo类上，以后都会应用此注解，此注解优先级最高。</font>
...
public class MsConfigure extends AbstractMsConfigure {
    ...
    //version 1.0.1
    //打入数据源至存储配置项中，打入后自动激活日志转储功能，此功能需要提供库中的表结构
    @Override
    public DataSource configDataSource() {
        DruidDataSource druidDataSource = new DruidDataSource();
        druidDataSource.setDriverClassName("com.mysql.jdbc.Driver");
        druidDataSource.setUrl("jdbc:mysql://localhost:3306/mytest");
...
<font color=#FF0000>v1.0.1可通过实现AbstractStoreRecordConfigure进行日志处理，会发送一个DownloadReword对象进入该类的实现对象，该类中的dataSource未指定泛型，可通过AbstractMsConfigure引入，引入对象未定，用户可进行自定义处理，组件提供数据库的入库操作，将DataSource引入后日志组件会进行工作，即只有configDataSource返回不为空的情况下，日志组件会正常工作，至于这个数据会流转至何种渠道，可通过用户自定义save方法实现。</font>
...
配置是通过json实现的，第一层的json的key：value集就是每一个映射实体，key为映射环境下的唯一id，如同pojo映射中的MsOperator的value值，内部为英文字段和中文字段的映射关系。  
<font color=#AF5080 size=3>双$标识此字段导出时需要调用ITransFunctionContainer中的指定双$后面的方法，注意标注在中文字段的最后，不然导致方法无法找到并且双$后面的字段无法展示，如果实在需要用到双$,可通过\\\\进行强制转换。  
className为指定该映射为某一个Pojo类的映射，如果不需要不要填写，如果填写后此映射就如同在pojo类中注解一样，返回的List中元素的类型会是Pojo对象而不是Map，和双$相同，可以被转义</font>
